...
La solución está construida con una arquitectura desacoplada y moderna que integra:
- **Backend Robusto:** API RESTful en **.NET 9 (C#)** con seguridad basada en **JWT** y acceso a datos optimizado mediante **SQL Server** y procedimientos almacenados.
- **Frontend Dinámico:** Single Page Application (SPA) en **JavaScript moderno (ES6+)** con diseño UI/UX premium, animaciones fluidas, reproductor de audio/video integrado y modo oscuro.
- **Almacenamiento Multimedia Cloud:** Integración con **Supabase Storage** y CDN para archivos de audio, video e imágenes sin límite de almacenamiento.
...
| 📱 **Feed Multimedia** | Muro interactivo estilo red social con publicaciones, carruseles de fotos/videos/audios, "me gusta" y comentarios. |
...
| 🛡️ **Módulo Administrativo** | Supervisión, control y métricas de la plataforma. |
...
- **Estilos & UI:** CSS3 Vanilla (Variables CSS, Glassmorphism, Layouts Flexbox/Grid, Animaciones suaves)
- **Iconografía & Fuentes:** FontAwesome + Google Fonts (Inter / Outfit)
...
- **ImgBB CDN:** Red de entrega rápida de imágenes.
...
1. **[.NET 9 SDK](https://dotnet.microsoft.com/download/dotnet/9.0)** (o .NET 8+).
2. **[Microsoft SQL Server](https://www.microsoft.com/sql-server/)** (Express, Developer o Standard) junto con **[SQL Server Management Studio (SSMS)](https://learn.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms)** o **Azure Data Studio**.
3. **[Visual Studio Code](https://code.visualstudio.com/)** (con la extensión **Live Server** instalada) o cualquier editor de tu preferencia.
4. 🌐 **Conexión activa a Internet:** Necesaria para la carga/reproducción multimedia en la nube (Supabase Storage y CDNs).
...
### 2️⃣ Paso 2: Configurar la Cadena de Conexión en el Backend
...
### 3️⃣ Paso 3: Ejecución de la API Backend (.NET)
...
### 4️⃣ Paso 4: Ejecución del Frontend
...
4. Tu navegador abrirá automáticamente la aplicación en `http://127.0.0.1:5500` (o el puerto configurado por Live Server).
...
## 🔒 Variables y Configuración de Seguridad
